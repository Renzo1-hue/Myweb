...
class SendOTPView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            print(f"OTP for {email}: {code}")  # For testing
...
class VerifyOTPView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            return Response({"success": True, "message": "OTP verified successfully", "email": serializer.validated_data['email']})
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
# views.py (Replace the existing dashboard_view function)
@login_required
def dashboard_view(request):
    ...
    # Count only mobile app user registrations (KEEPING OLD DATA FOR NOW, BUT GRAPH WILL BE REPLACED)
    user_registrations = (
    ...
    # Prepare graph data (Keeping old variables to prevent other code breaks)
    months = [entry['month'].strftime('%B') for entry in user_registrations]
    counts = [entry['count'] for entry in user_registrations]
    max_users = max(counts) if counts else 1
    
    # Generate SVG path data (Keeping old variables to prevent other code breaks)
    path_points = []
    ...
    # --- NEW: Reward Distribution Data (For Donut Chart) ---
    reward_distribution = (
        ClaimRequest.objects.filter(status='approved')
        .values('reward__name')
        .annotate(claimed_count=Count('reward'))
        .order_by('-claimed_count')
    )
    
    # Prepare data for JavaScript (JSON dump is essential for passing complex lists safely)
    reward_labels = [entry['reward__name'] for entry in reward_distribution]
    reward_counts = [entry['claimed_count'] for entry in reward_distribution]
    
    # Note: Using json.dumps to pass the list structures to the JavaScript side
    ...
    context = {
        ...
        'reward_labels': json.dumps(reward_labels),
...
# views.py - Updated api_get_claim_history function
@csrf_exempt
@require_http_methods(["POST"])
